...
<section class="hero2" id="hero">
    <div class="hero2__bg" aria-hidden="true">
        ...
        /* Hero player image (online URL). Rendered as a glowing white silhouette so any
           royalty-free footballer/silhouette PNG looks on-brand (and avoids likeness issues). */
        .wcfx__player{position:absolute;right:3%;bottom:0;height:90%;max-height:540px;object-fit:contain;object-position:bottom right;opacity:.9;filter:brightness(0) invert(1) drop-shadow(0 0 34px rgba(22,199,132,.45));animation:wcrise 1s cubic-bezier(.2,.7,.2,1) both;pointer-events:none}
        @keyframes wcrise{from{opacity:0;transform:translateY(34px)}to{opacity:.9;transform:translateY(0)}}
        ...
        <div class="wcfx">
            ...
            <?php /* Paste ANY online transparent PNG/SVG URL below (royalty-free / silhouette recommended). */ ?>
            <?php $wcPlayer = 'https://upload.wikimedia.org/wikipedia/commons/4/4c/Football_pictogram.svg'; ?>
            <?php if (!empty($wcPlayer)): ?>
            <img class="wcfx__player" src="<?= $wcPlayer ?>" alt="" aria-hidden="true" onerror="this.remove()">
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
